<?php
/*
 * Elementor Restaurant & Cafe Addon for Elementor Button Widget
 * Author & Copyright: NicheAddon
*/

namespace Elementor;

if (!isset(get_option( 'rcafe_bw_settings' )['nbeds_restaurant_button'])) { // enable & disable

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

class Restaurant_Elementor_Addon_Button extends Widget_Base{

	/**
	 * Retrieve the widget name.
	*/
	public function get_name(){
		return 'narestaurant_basic_button';
	}
	...
	*/
	public function get_title(){
		return esc_html__( 'Restaurant Button', 'restaurant-cafe-addon-for-elementor' );
	}
	...
	*/
	...
	/**
	 * Register Restaurant & Cafe Addon for Elementor Button widget controls.
	...
	*/
	protected function _register_controls(){
		...
		$this->start_controls_section(
			'section_btn',
			[
				'label' => __( 'Button Options', 'restaurant-cafe-addon-for-elementor' ),
			]
		);
		$this->add_control(
			'btn_text',
		...
		$this->add_responsive_control(
			...
			[
				...
				'default' => 'left',
				'selectors' => [
					'{{WRAPPER}} .narep-btn-wrap' => 'text-align: {{VALUE}};',
				],
			]
		);
		...
		// Button
			...
			$this->add_control(
				...
				[
					...
					'size_units' => [ 'px', '%', 'em' ],
					'selectors' => [
						'{{WRAPPER}} .narep-btn-wrap' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
				]
			);
			$this->add_control(
			...
			$this->add_control(
				'btn_padding',
				[
					'label' => __( 'Button Padding', 'restaurant-cafe-addon-for-elementor' ),
					'type' => Controls_Manager::DIMENSIONS,
					'size_units' => [ 'px', '%', 'em' ],
					'selectors' => [
	...
	*/
	protected function render() {
		...
		$button = $btn_link ? '<div class="narep-btn-wrap"><a href="'.esc_url($btn_link).'" class="narep-btn" '.$btn_link_attr.'>'.esc_html($btn_text).'</a></div>' : '';
...
Plugin::instance()->widgets_manager->register_widget_type( new Restaurant_Elementor_Addon_Button() );
