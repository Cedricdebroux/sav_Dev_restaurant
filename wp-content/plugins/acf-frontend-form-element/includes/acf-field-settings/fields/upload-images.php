...
if( ! class_exists('acf_field_upload_images') ) :

class acf_field_upload_images extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		// vars
		$this->name = 'upload_images';
		$this->label = __("Gallery",'acf');
		$this->category = 'acfef-hidden';
		$this->defaults = array(
			'library'		=> 'all',
			'min'			=> 0,
			'max'			=> 0,
        ...
        add_action('wp_ajax_acf/fields/upload_images/add_attachment',				array($this, 'ajax_add_attachment'));
		add_action('wp_ajax_nopriv_acf/fields/upload_images/add_attachment',		array($this, 'ajax_add_attachment'));
	...
	*/
	...
	function ajax_add_attachment() {
		...
		// validate nonce
		if( !wp_verify_nonce($args['nonce'], 'acf_form') ) {
			wp_send_json_error();			
		} 
		...
		if( empty($_FILES['file']['name']) ) {		
            ...
            wp_send_json_success( [ 'id' => $attach_id, 'title' => $attachment[ 'post_title' ] ] );
    }
	...
	*/
	...
	function render_field( $field ) {
        ...
        $button_text = isset( $field['button_text'] ) ? $field['button_text'] : __( 'Add Images', 'acf-frontend-form-element' );
	...
	<div class="acf-gallery-main">
		...
		<div class="acf-gallery-attachments">
            ...
            <div class="image-preview-clone acf-hidden">
				...
				<div class="actions">
					<a class="acf-icon -cancel dark acf-gallery-remove" href="#" title="<?php _e('Remove', 'acf'); ?>"></a>
				</div>
            </div>
                ...
                <?php if( $uploader == 'basic' ): ?>
                    <li>
                        <label class="acf-basic-uploader file-drop">
                            <?php acf_file_input(array( 'name' => 'upload_images_input', 'id' => $field['id'], 'class' => 'images-preview', 'multiple' => 'true', 'accept' => "image/*" ) ); ?>
...
// initialize
acf_register_field_type( 'acf_field_upload_images' );
