...
class ACFS_Module extends Module
{
    ...
    public function load_acfef_dynamic_settings( $field )
    {
        ...
        /* $tag_value = \Elementor\Plugin::$instance->dynamic_tags->parse_tags_text( $field[ 'placeholder' ], $field, [ \Elementor\Plugin::$instance->dynamic_tags, 'get_tag_data_content' ] );
        
        		$field[ 'placeholder' ] = $tag_value;  */
        return $field;
    }
    ...
    public function acfef_load_gallery_field( $field )
    {
        ...
        if ( !isset( $field['custom_product_gallery'] ) ) {
            return $field;
        }
    ...
    public function acfef_load_password_field( $field )
    {
        if ( !isset( $field['edit_user'] ) ) {
            return $field;
        }
    ...
    )
    {
        if ( isset( $field['add_edit_post'] ) && $field['add_edit_post'] == 1 ) {
            $title .= '<a href="#" class="acf-icon -pencil small dark edit-post" 	data-name="edit_item"></a>';
        }
        return $title;
    }
    ...
    public function acfef_add_post_button( $field )
    {
        ...
        if ( isset( $field['add_edit_post'] ) && $field['add_edit_post'] == 1 ) {
			...
			<div class="margin-top-10 acf-actions">
				<a class="acf-button button button-primary" href="#" data-name="add_item"><?php 
    ...
    public function acfef_ajax_add_form()
    {
        ...
        $hidden_fields = [
            ...
            'main_action' => ( is_numeric( $args['form_action'] ) ? 'edit_post' : 'new_post' ),
        ];
        if ( is_admin() ) {
        ...
        if ( $field['post_form_template'] != 'none' ) {
            ...
            if ( is_numeric( $args['form_action'] ) ) {
                ...
                if ( empty($field['post_type']) ) {
                    $form_args['post_fields']['post_type'] = 'post';
                    $post_type_choices = $all_post_types;
                } elseif ( count( $field['post_type'] ) > 1 ) {
                    $form_args['post_fields']['post_type'] = $field['post_type'][0];
                    $post_type_choices = [];
    ...
    )
    {
        ...
        if ( isset( $field['custom_password'] ) && $field['custom_password'] == 1 ) {
            ...
            if ( (int) esc_attr( $_POST['password-strength'] ) < (int) esc_attr( $_POST['required-strength'] ) ) {
                if ( !$field['required'] && $value == '' || isset( $_POST['edit_user_password'] ) && $_POST['edit_user_password'] == 1 ) {
                    return $is_valid;
                }
    ...
    {
        $status = ( defined( WP_FS__DEV_MODE ) ? '' : '.min' );
        wp_enqueue_script(
            'acfef-recaptcha',
    ...
    public function __construct()
    {
        ...
        add_filter( 'acf/load_field/type=password', [ $this, 'acfef_load_password_field' ] );
        add_filter( 'acf/load_field', [ $this, 'load_acfef_dynamic_settings' ], 3 );
        ...
        //add_filter( 'acf/load_value/type=password',  [ $this, 'acfef_load_password_value' ], 10, 3);
        add_filter(
        ...
        add_filter(
            'acf/load_value/type=product_images',
        ...
        add_filter(
            'acf/update_value/type=product_images',
            [ $this, 'acfef_update_gallery_value' ],
            9,
            3
        );
