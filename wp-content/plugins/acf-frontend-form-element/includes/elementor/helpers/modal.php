...
function acfef_before_element_render( $settings, $wg_id  ){
    if( ! isset( $settings[ 'show_in_modal' ] ) || ! $settings[ 'show_in_modal' ] ){
        ...
        $show_modal = 'hide';
        if( isset( $_GET[ 'modal' ] ) && (
		   ( isset( $_GET[ 'form_id' ] ) && $wg_id == $_GET[ 'form_id' ] )
	    || ( isset( $_GET[ 'updated' ] ) && strpos( $_GET[ 'updated' ], $wg_id ) != false )
		) )	$show_modal = 'show';			
			
        $before = '<button class="acfef-edit-button edit-button" onClick="openModal(\'' .$wg_id.get_the_ID(). '\')" >'; 
        ...
        $before .= '<div id="modal_' .$wg_id.get_the_ID(). '" class="modal edit-modal ' .$show_modal. '">
                <div class="modal-content"> 
                    <div class="modal-inner"> 
                    <span onClick="closeModal(\'' .$wg_id.get_the_ID(). '\')" class="acf-icon -cancel close"></span>
