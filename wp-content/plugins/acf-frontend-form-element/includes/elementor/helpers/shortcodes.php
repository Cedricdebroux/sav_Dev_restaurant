            ...
            case 'user':
                ...
                if( $multiple && is_array( $value ) ){
                    ...
                    foreach( $value as $user ){
                        $return .= get_user_field( $term_field, $user );
                        if( $x < count($value) ) $return .= ', '; 
                        $x++;
                    }
                }else{
                    $return = get_user_field( $term_field, $value );
                }
                break;
            case 'post_object':
            case 'relationship':
				...
				case 'main_image':
					...
					if( strpos( $match, ':' ) !== false ){
						...
						if( $match_params[2] == 'image_link' ){
							...
							$value = '<div style="max-width:' .$match_params[2]. '"><img src="' . $post_thumb_url . '"/></div>';
						}
					}else{
