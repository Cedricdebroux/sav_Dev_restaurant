...
class SaveFields {
	...
	public function save_widget_fields( $widget, $post_id = '', $update = false ) {
		$widget_names = [
			'acf_ele_form', 'edit_post', 'edit_term', 'new_term', 'new_post', 'edit_product', 'new_product', 'new_comment', 'edit_user', 'new_user', 'edit_options', 'acf_form_fields' 
		];
		if( is_object( $widget ) ){
		...
		if( ! empty( $settings[ 'fields_selection' ] ) ){
			...
			if( isset( $action ) ){
				foreach( $settings[ 'fields_selection' ] as $form_field ){
					...
					$local_field = $action->get_fields_display( $form_field, $local_field, $edit_id, $wg_id, $settings );
