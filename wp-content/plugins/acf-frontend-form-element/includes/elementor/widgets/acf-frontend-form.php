...
class ACF_Frontend_Form_Widget extends Widget_Base
{
    ...
    public function show_widget( $wg_id, $settings, $form_args )
    {
        ...
        if ( 'logged_in' == $settings['who_can_see'] ) {
            ...
            if ( !is_user_logged_in() ) {
                ...
                if ( isset( $user_id[1] ) && $user_id[1] == $active_user->ID && !$settings['by_role'] ) {
                    return true;
                }
                ...
                if ( $user_roles ) {
                    if ( is_array( $settings['by_role'] ) ) {
                        if ( count( array_intersect( $settings['by_role'], (array) $active_user->roles ) ) != false ) {
                            $by_role = true;
                        }
                    }
                } else {
                ...
                if ( !isset( $settings['dynamic'] ) && !isset( $settings['dynamic_manager'] ) ) {
                    $dynamic = true;
                } else {
    ...
    )
    {
        ...
        if ( 'new_post' == $settings['main_action'] ) {
            ...
            if ( $can_edit && ($settings['redirect_action'] == 'edit' || isset( $_GET['draft'] )) ) {
            ...
            $status = ( isset( $settings['new_post_status'] ) && $settings['new_post_status'] != 'no_change' ? $settings['new_post_status'] : 'publish' );
            $tax_input = [];
        ...
        if ( 'edit_post' == $settings['main_action'] ) {
            ...
            $form_args['action'] = 'post';
            $status = ( isset( $settings['new_post_status'] ) ? $settings['new_post_status'] : 'no_change' );
            $action = 'post';
            $args = array();
            if ( $status != 'no_change' ) {
        ...
        if ( 'new_user' == $settings['main_action'] ) {
            ...
            if ( $object_id ) {
                ...
                if ( !$can_edit || $settings['redirect_action'] == 'clear' ) {
        ...
        if ( 'new_term' == $settings['main_action'] ) {
            $form_args['post_id'] = 'new_term';
            $args = [
                'taxonomy' => $settings['new_term_taxonomy'],
            ];
            $action = 'term';
        }
