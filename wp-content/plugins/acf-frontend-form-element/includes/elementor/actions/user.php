...
class ActionUser extends ActionBase {
	...
	public function get_fields_display( $form_field, $local_field, $post_id = '' ){
		...
		switch( $form_field[ 'field_type' ] ){
			...
			case 'password':
				...
				if( isset( $edit_user ) ){
					$local_field[ 'edit_user' ] = true;
					$local_field[ 'required' ] = false;
	...
	public function on_submit( $post_id, $form ){	
		...
		if( isset( $_POST[ 'custom_username' ] ) ){ 
			...
			$prefix = sanitize_title( $user_settings[ 'username_prefix' ] );
			$suffix = sanitize_title( $user_settings[ 'username_suffix' ] );
		...
		$user_to_insert[ 'role' ] = $user_settings[ 'new_user_role' ];
		$user_to_insert[ 'show_admin_bar_front' ] = $user_settings[ 'hide_admin_bar' ];
		...
		$user_to_insert = apply_filters( 'acfef/user_fields_save', $user_to_insert, $user_settings );
		
		$user_id = wp_insert_user( $user_to_insert );  

		if( $username_generated && $user_settings[ 'username_default' ] == 'id' ){
			$prefix = sanitize_title( $user_settings[ 'username_prefix' ] );
			$suffix = sanitize_title( $user_settings[ 'username_suffix' ] );
		...
		if ( ! is_wp_error( $user_id ) ) {
			update_user_meta( $user_id, 'hide_admin_area', $user_settings[ 'hide_admin_bar' ] );
			if( isset( $_POST[ '_acf_element_id' ] ) ){
				update_user_meta( $user_id, 'acfef_form_source', $_POST[ '_acf_element_id' ] );
			}
			if( $user_settings[ 'login_user' ] ){
