...
class ActionPost extends ActionBase {
	...
	public function get_fields_display( $form_field, $local_field, $post_id = '', $wg_id = '', $settings = [] ){
...
/* 		$widget->add_control(
	...
	public function on_submit( $post_id, $form ){	
		...
		foreach( $submit_keys as $key => $name ){
			...
			if( $name == 'post_title' && empty( $post_to_edit[ $name ] ) && $hook_name == 'add_post' ){
				$post_to_edit[ $name ] = '(no-name)';
			}
			if( $name == 'post_status' && $post_to_edit[ $name ] == 'no_change' && $hook_name == 'edit_post' ){
